...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if not check_procedure(procedure):
        raise SchemeError('not a valid Scheme procedure')
    '''
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        evaluated = scheme_eval(expressions.first, env, True)
        return evaluated
    else:
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('argument lengths do not match up')
        else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        function_arguments = target.rest
        call_expression = expressions.rest
        binding = do_lambda_form(Pair(function_arguments, call_expression), env)
        env.define(variable, binding)
        return variable
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is nil:
        evaluation = scheme_eval(expressions.first, env, True)
    else:
        evaluation = scheme_eval(expressions.first, env)
    if evaluation and expressions.rest is nil:
        return evaluation
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        bind_result = Pair(scheme_eval(bindings.first.rest.first, env), bind_result)
        bindings = bindings.rest
    check_formals(formals)
    return env.make_child_frame(formals, bind_result)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        variable = target.first
        binding = MacroProcedure(target.rest, expressions.rest, env)
        env.define(variable, binding)
        return variable
    else:
        bad_target = target.first if isinstance(target, Pair) else target
