...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        for branch in branches(t):
            branch_list.append(replace_leaf(branch, old, new))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, guess):
        ...
        if len(incorrect_guesses) == 3:
            return "Your account is locked. Attempts: " + str(incorrect_guesses)
        elif guess != password:
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    checker = withdraw(0, old_password)
    if type(checker) == str:
        return checker
    def joint(amount, password1):
        if password1 == new_password or password1 == old_password:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, password1)
    return joint
